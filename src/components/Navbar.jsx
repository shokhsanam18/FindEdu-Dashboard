...
export function ComplexNavbar() {
  ...
  const getSearchData = () => {
    try {
      ...
      return [
        ...fields.map(item => ({ ...item, type: "field" })),
        ...majors.map(item => ({ ...item, type: "major" })),
        ...users.map(item => ({ ...item, type: "user" })),
        ...centers.map(item => ({ ...item, type: "center" })),
        ...regions.map(item => ({ ...item, type: "region" })),
        ...subjects.map(item => ({ ...item, type: "subject" })),
        ...categories.map(item => ({ ...item, type: "category" })),
        ...themeOptions.map(item => ({ ...item, type: "theme" }))
      ];
    } catch (error) {
  ...
  const handleSelect = (item) => {
    ...
    if (item.link) {
      navigate(item.link);
    } else if (routes[item.type]) {
      navigate(routes[item.type]);
    } else {
      console.warn("No route defined for item:", item);
    }
  };
        ...
        <div className="w-full flex flex-col items-center relative p-1">
          ...
          >
            <input
              ...
              placeholder="Search fields, majors, users, centers..."
              value={query}
          ...
          {query.length > 0 && suggestions.length > 0 && (
            <div className="absolute top-full mt-2 w-full max-w-lg bg-white shadow-lg rounded-md border border-gray-300 overflow-hidden max-h-60 overflow-y-auto z-50 animate-fade-in">
              {suggestions.map((item, index) => (
                <div
                  key={index}
                  className="p-3 hover:bg-violet-100 cursor-pointer transition-all flex items-center gap-2"
